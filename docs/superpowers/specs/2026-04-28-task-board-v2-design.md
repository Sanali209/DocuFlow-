...
### Обязательные
- [x] 2 таба: "Производство" и "Моя корзина"
- [x] Таб "Производство": иерархия Project→WorkItem→TaskGroup→TaskItem
- [x] Двухстрочные строки на всех уровнях иерархии
- [x] Раскрытие/сворачивание уровней с сохранением в DB (ViewState)
- [x] Переназначение WorkItem между Project'ами
- [x] Создание/удаление/редактирование Project и WorkItem
- [x] Регистрация WorkItem вручную (без сканера)
- [x] Управление TaskGroup: создание, разбиение, назначение на узел
- [x] Комплексные фильтры с пресетами (ViewPreset)
- [x] Omnisearch работает по всем уровням иерархии
- [x] Таб "Моя корзина": TaskGroup'ы на узле + передача смены
- [x] Передача смены: форма (кнопка → разворот) + входящий баннер + "Принято"
- [x] TaskGroup — полноценная DB-сущность, batch_group_id удалён
