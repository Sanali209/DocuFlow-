    ...
    if 'ui.notify' in content:
        ...
        if 'NotifyHelper' not in content:
            ...
            if 'from nicegui import ui' in content:
                content = content.replace('from nicegui import ui', 'from nicegui import ui\nfrom docuflow.lib.widgets.ui_utils import NotifyHelper')
            else:
        ...
        content = re.sub(r'ui\.notify\((.*?),\s*(?:color|type)="warning"(?:,\s*icon=".*?")?\)', r'NotifyHelper.warning(\1)', content, flags=re.DOTALL)

        # 2. color="negative" or type="negative" or color="red"
        content = re.sub(r'ui\.notify\((.*?),\s*(?:color|type)="(?:negative|red)"(?:,\s*icon=".*?")?\)', r'NotifyHelper.error(\1)', content, flags=re.DOTALL)

        # 3. color="positive" or type="positive" or color="emerald" or color="green"
        content = re.sub(r'ui\.notify\((.*?),\s*(?:color|type)="(?:positive|emerald|green)"(?:,\s*position=".*?")?\)', r'NotifyHelper.success(\1)', content, flags=re.DOTALL)

        # 4. type="info"
        content = re.sub(r'ui\.notify\((.*?),\s*type="info"\)', r'NotifyHelper.info(\1)', content, flags=re.DOTALL)

        # 5. color="yellow" or color="orange" -> warning
        content = re.sub(r'ui\.notify\((.*?),\s*color="(?:yellow|orange)"\)', r'NotifyHelper.warning(\1)', content, flags=re.DOTALL)
