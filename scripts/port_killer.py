...
def get_all_processes() -> dict[str, str]:
    ...
    try:
        result = subprocess.run(
            ["tasklist", "/FO", "CSV"], capture_output=True, text=True, encoding="cp866"
        )
        processes = {}
            ...
            if "LISTENING" in line:
                ...
                if len(parts) >= 5:
                    ...
                    if ":" in addr_port:
                        ...
                        pid = parts[4]
                        ports.append({"port": port, "pid": pid, "status": "LISTENING"})
        return ports
    except Exception as e:
...
def kill_process(pid: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ["taskkill", "/PID", pid, "/F"], capture_output=True, text=True, encoding="cp866"
        )
...
class PortKillerApp:
    def __init__(self, root: tk.Tk):
        ...
        ttk.Button(search_frame, text="Убить процесс", command=self.kill_selected).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def _update_tree(self, ports: list[dict], processes: dict[str, str]):
        ...
        for port_info in ports:
            ...
            self.tree.insert(
                "",
                tk.END,
                values=(port_info["port"], port_info["pid"], process_name, port_info["status"]),
            )
    ...
    def kill_selected(self):
        ...
        if messagebox.askyesno(
            "Подтверждение", f"Завершить процесс?\n\nПорт: {port}\nPID: {pid}\nПроцесс: {process}"
        ):
