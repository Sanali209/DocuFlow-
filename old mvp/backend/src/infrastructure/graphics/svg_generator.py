...
from ..parsers.gnc_parser import GNCPart
...
class SVGGenerator:
    ...
    def generate_thumbnail(
        self, part: GNCPart | None, output_path: str, width: int = 200, height: int = 200
    ) -> tuple[float, float]:
        """
        ...
        for contour in part.contours:
            ...
            for cmd in contour.commands:
                ...
                if is_first_point:
                    ...
                    cmd.type == "G01"
                    or (cmd.command == "G" and cmd.value == 1)
                    or cmd.type == "MODAL"
                    or cmd.type == "G41"
                    or cmd.type == "G40"
                ):
                    ...
                    cmd.type == "G02"
                    or cmd.type == "G03"
                    or (cmd.command == "G" and cmd.value in [2, 3])
                ):
                    # Arc
                    if prev_x is not None and prev_y is not None:
                        ...
                        sweep_flag = (
                            0 if is_clockwise else 1
                        )  # SVG sweep: 0=counterclockwise, 1=clockwise
