...
def read_documents(
    ...
    material_id: Optional[int] = Query(None, alias="material"), # Mapping "material" query param to material_id
...
async def save_as_new_order(data: dict = Body(...), service: DocumentService = Depends(get_document_service)):
    print(f"Received save_as_new_order request: Name={data.get('name')}, Sheets={len(data.get('sheets', []))}")
    try:
...
def open_document_folder(document_id: int, service: DocumentService = Depends(get_document_service)):
    import os
    import sys
    import subprocess
    ...
    if not folder_path or not os.path.exists(folder_path):
         raise HTTPException(status_code=404, detail="Could not determine a valid folder for this document")
