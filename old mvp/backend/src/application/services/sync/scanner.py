...
class DirectoryScanner:
    def __init__(self, processor: SyncProcessor, order_scanner=None):
        self.processor = processor
        self.order_scanner = order_scanner
        self.suffixes = ['_801', 'to801', '_to801']

    def scan(self, root_path: str, source_type: str):
        # 1. Run Order Scanner if configured
        if self.order_scanner and (source_type == "Sidra" or source_type == "Michtaw"):
             self.order_scanner.scan_and_process(root_path, source_type)
        
        # 2. Continue with standard scan (if needed, or maybe OrderScanner handles it all?)
        # The requirement says: "scaner faind order folder in "in" folder move to "registred"..."
        # So for Sidra/Michtaw, we might ONLY want OrderScanner logic?
        # But we should keep existing logic for other folders if any.
