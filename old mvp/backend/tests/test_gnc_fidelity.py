...
SAMPLE_PATH = (
    "testing/sidra_test/sidra 3455/06-02-SIDRA-351501-SHLAV-1-23.12.2024-SS 1.4003-1.5.GNC"
)
...
def test_gnc_multi_part_nesting():
    ...
    from backend.gnc_parser import GNCSheet
    ...
    for contour in source_part.contours:
        if any(
            "(==== CONTOUR" in cmd.original_text.upper()
    ...
    print(
        f"Generated {len(contour_lines)} contour lines (Source part had {n_source_part_contours})"
    )

    assert len(contour_lines) == 2 * n_source_part_contours, (
        f"Expected {2 * n_source_part_contours} contours, got {len(contour_lines)}"
    )
    ...
    assert all(n_codes[i] < n_codes[i + 1] for i in range(len(n_codes) - 1)), (
        "N-codes are not strictly increasing"
    )
    ...
    for m1, m2 in zip(p1_moves, p2_moves):
        ...
        if ssd_match1 and ssd_match2:
            ...
            if n2_match:
                ...
                assert n2_val == ssd2_val, (
                    f"SSD call {ssd2_val} does not match N-code {n2_val} in line: {m2}"
                )
...
def test_gnc_sidra_roundtrip():
    ...
    for code in critical_codes:
        ...
        assert found_orig == found_gen, (
            f"Critical code {code} parity failed. Orig: {found_orig}, Gen: {found_gen}"
        )
    ...
    for o_move, g_move in zip(orig_moves[:20], gen_moves[:20]):
        for axis in ["X", "Y", "I", "J"]:
            ...
            if o_m and g_m:
                assert abs(float(o_m.group(1)) - float(g_m.group(1))) < 0.001, (
                    f"Coordinate mismatch: {o_move} vs {g_move}"
                )
