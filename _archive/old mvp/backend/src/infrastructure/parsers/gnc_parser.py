...
class GNCCommand(BaseModel):
    ...
    command: str | None = None  # G, M, T
    value: float | None = None  # 0, 1, 30, etc.
    x: float | None = None
    y: float | None = None
    i: float | None = None
    j: float | None = None
...
class GNCSheet(BaseModel):
    ...
    program_width: float | None = None  # Position 1: program width in mm
    program_height: float | None = None  # Position 2: program height in mm
    cut_count: int | None = None  # Position 4: number of times to cut
...
class GNCParser:
    ...
    def parse(self, content: str, filename: str = "") -> GNCSheet:
        ...
        sheet_detail_pattern = re.compile(
            r"\(\*SHEET\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+(\d+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s*\)",
            re.IGNORECASE,
        )
        ...
        for i, line in enumerate(lines):
            ...
            if p_code_801_match:
                if current_part is None:
                    ...
                    default_name = (
                        os.path.basename(filename).replace(".gnc", "").replace(".GNC", "")
                    )
            ...
            if commands_found:
                ...
                for prefix, val_str in commands_found:
                    ...
                    value = float(val_str)
                    type_str = (
                        f"{prefix}{int(value):02d}" if prefix in ["G", "M"] else f"{prefix}{value}"
                    )
    ...
    def _post_process(self, sheet: GNCSheet):
        ...
        for part in sheet.parts:
            ...
            for contour in part.contours:
                ...
                motion_cmds = [
                    c for c in contour.commands if c.command == "G" and c.value in [0, 1, 2, 3]
                ]
                # Also count MODAL commands that have coordinates (treated as G01 usually)
                motion_cmds += [
                    c
                    for c in contour.commands
                    if c.type == "MODAL" and (c.x is not None or c.y is not None)
                ]
