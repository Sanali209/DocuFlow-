...
def mock_engine(tmp_path):
    ...
    from docuflow.domain.entities.production import WorkItem, TaskItem, TaskPart, PartLibrary, MaterialType, Project, WorkLog
...
async def test_scan_now_disabled(scanner_system):
    ...
    scanner_system._admin.get_node_settings = MagicMock(return_value={
        "enabled": False,
        "sidra_scan_path": "/some/path"
    })
...
async def test_scan_all_with_nonexistent_path(scanner_system):
    ...
    # Setup
    settings = FolderScannerSettings(
        sidra_scan_path="/nonexistent/path",
        enabled=True
    )
