...
def test_admin_role_delete_protection():
    ...
    assert not session_mock.delete.called, "CRITICAL: 'admin' role deletion should be blocked in session"
    assert not orchestrator_mock.broadcast_command.called, "CRITICAL: 'admin' role deletion should not be broadcast"
...
def test_admin_role_upsert_protection():
    ...
    assert not orchestrator_mock.broadcast_command.called, "CRITICAL: 'admin' role modification should not be broadcast"
