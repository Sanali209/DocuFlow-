...
async def test_e2e_cluster_failover_and_sync(tmp_path):
    ...
    config_a = Config(
        node_id="NODE_A", database_url=f"sqlite:///{tmp_path}/node_a.db", **common_params
    )

    config_b = Config(
        node_id="NODE_B", database_url=f"sqlite:///{tmp_path}/node_b.db", **common_params
    )
    ...
    assert follower_coord.is_leader is True, (
        f"Follower {follower_coord._node_id} failed to take over"
    )
