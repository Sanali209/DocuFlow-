    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Constant) and isinstance(node.value, str):
            ...
            if (len(value) <= 1 or 
                ...
                value.isupper()):  # Already a constant
                continue
...
class TestNoMagicNumbersInSystem:
    ...
    def test_no_magic_number_4096_for_md5(self):
        ...
        for i, line in enumerate(lines):
            ...
            if "4096" in line:
                assert False, \
                    f"Line {i+1}: Magic number 4096 should use MD5_CHUNK_SIZE constant"
...
class TestAllMethodsHaveDocstrings:
    ...
    def test_di_methods_have_docstrings(self):
        ...
        assert not missing_docstrings, \
            f"Methods without docstrings in di.py: {missing_docstrings}"
