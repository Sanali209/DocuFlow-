...
from docuflow.domain.entities.production import WorkItemType, TaskItem, MaterialType
...
async def test_full_workshop_pipeline(db_session: Session, config: Config):
    ...
    work_item = work_item_sys.create_work_item(
        folder_name="SMOKE-BATCH-001",
        folder_path="/workshop/smoke",
    ...
    registry.register(ReportDataBlock(
        name="downtime_summary", label="Downtime", params=[], 
        query_fn=lambda s, p: {"Mechanical": 10.5, "Electrical": 5.0}
    ))
    registry.register(ReportDataBlock(
        name="incident_log", label="Incidents", params=[], 
        query_fn=lambda s, p: []
    ))
    ...
    html_preview = report_sys.generate_html_preview(
        report_sys.TEMPLATE_SHIFT_SUMMARY, 
