...
class TestWorkItemSystemCreate:
    ...
    def test_create_work_item_with_project_id(self, system: WorkItemSystem, session: Session):
...
class TestWorkItemSystemList:
    ...
    def test_list_all_work_items(self, system: WorkItemSystem, default_project: Project):
        system.create_work_item(
            folder_name="SIDRA-1", folder_path="p1", item_type=WorkItemType.SIDRA
        )
        system.create_work_item(
            folder_name="MIHTAV-1", folder_path="p2", item_type=WorkItemType.MIHTAV
        )
...
class TestWorkItemSystemLifecycle:
    ...
    def test_update_production_status(self, system: WorkItemSystem, default_project: Project):
        wi = system.create_work_item(
            folder_name="SIDRA-STATUS", folder_path="p", item_type=WorkItemType.SIDRA
        )
    ...
    def test_invalid_transition_raises(self, system: WorkItemSystem, default_project: Project):
        wi = system.create_work_item(
            folder_name="SIDRA-FAIL", folder_path="p", item_type=WorkItemType.SIDRA
        )
        # NEW -> DONE is illegal
        with pytest.raises(ValueError):
