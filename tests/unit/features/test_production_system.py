...
from docuflow.domain.entities.production import Project, WorkItem, TaskItem, ProductionUnit, WorkLog, WorkLogType, WorkItemType
...
def test_create_production_unit(production_system, session):
    ...
    wi = WorkItem(folder_name="PROD-TEST-UNIQUE", folder_path="test/path", project_id=project_id, sidra_number="S-1", work_item_type=WorkItemType.SIDRA)
    session.add(wi)
    session.flush()
...
def test_split_pallet(production_system, session):
    ...
    wi = WorkItem(folder_name="SPLIT-TEST-UNIQUE", folder_path="test/path", project_id=project_id, sidra_number="S-2", work_item_type=WorkItemType.SIDRA)
    session.add(wi)
    session.flush()
    ...
    # Split 30 from 100
    new_unit = production_system.split_production_unit(original.id, move_quantity=30, author="worker")
...
def test_merge_pallets(production_system, session):
    ...
    wi = WorkItem(folder_name="MERGE-TEST-UNIQUE", folder_path="test/path", project_id=project_id, sidra_number="S-3", work_item_type=WorkItemType.SIDRA)
    session.add(wi)
    session.flush()
    ...
    merged = production_system.merge_production_units(
        source_pallet_ids=[p1.id, p2.id], 
        target_pallet_id=p3.id, 
