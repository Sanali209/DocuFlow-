...
def test_inverse_traceability(parts_system: PartLibrarySystem, session: Session):
    ...
    wi = WorkItem(folder_name="PROJECT-X", folder_path="path/to/x", sidra_number="S-123", project_id=proj.id, work_item_type=WorkItemType.SIDRA)
    session.add(wi)
    session.flush()
...
def test_part_templates_crud(parts_system: PartLibrarySystem, session: Session):
    parts_system.create_part_template(sku="SKU-T", note_message="Dangerous corner", severity_level="critical")
