...
from docuflow.domain.entities.production import (
    MaterialType, MaterialStock, MaterialAudit, Reservation, 
    TaskItem, WorkItem, WorkItemType, TaskItemStatus
)
...
def test_perform_write_off_handles_negative_stock(inventory_system, session, mock_sdk):
    ...
    wi = WorkItem(folder_name="TEST", folder_path="path", project_id=1, sidra_number="S-1", work_item_type="laser")
    session.add(wi)
    session.flush()
    session.refresh(wi)
    
    task = TaskItem(work_item_id=wi.id, file_name="part.gnc", file_path="path", status=TaskItemStatus.PLANNED, mat_type_id=mt.id)
    session.add(task)
    session.flush()
    ...
    audit = session.exec(select(MaterialAudit).where(MaterialAudit.ref_task_item_id == task.id)).first()
