...
def test_list_critical_filter(consumable_system: ConsumableSystem, session: Session):
    ...
    consumable_system.restock(
        consumable_system.get_consumable_by_name("OK").id, quantity_delta=10.0
    )

    consumable_system.create_consumable("CRITICAL", min_quantity=5.0)
    consumable_system.restock(
        consumable_system.get_consumable_by_name("CRITICAL").id, quantity_delta=2.0
    )
