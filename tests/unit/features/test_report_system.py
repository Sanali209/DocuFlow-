...
from docuflow.features.reports.system import ReportSystem, ReportRegistry, ReportDataBlock, BlockParam
...
def test_registry_registration(registry: ReportRegistry):
    def dummy_query(s, p): return [{"val": 1}]
...
def test_generate_html_with_blocks(report_system: ReportSystem, registry: ReportRegistry, session: Session):
    # Register a data block
    registry.register(ReportDataBlock(
        name="stats",
        label="Stats",
        params=[],
        query_fn=lambda s, p: [{"count": 42}]
    ))
    
    # Save a template that uses the block
    template = ReportTemplate(
        name="my_report",
        template_html="Count: {{ blocks.stats()[0].count }}"
    )
...
def test_generate_html_with_params(report_system: ReportSystem, registry: ReportRegistry, session: Session):
     registry.register(ReportDataBlock(
        name="echo",
        label="Echo",
        params=[],
        query_fn=lambda s, p: [{"received": p.get("val")}]
    ))
     
     template = ReportTemplate(
        name="echo_report",
        template_html="Val: {{ blocks.echo(val=params.custom)[0].received }}"
    )
     session.add(template)
     session.commit()
