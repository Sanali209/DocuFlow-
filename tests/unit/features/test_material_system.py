...
def test_receive_material_batch_creates_audit(inventory_system: InventorySystem, session: Session):
    ...
    stock = inventory_system.receive_material_batch(mt.id, quantity=10, batch_code="B1", location="A1")
    
    assert stock.quantity == 10
    assert stock.batch_code == "B1"
    
    # Verify audit
    audit = session.exec(select(MaterialAudit).where(MaterialAudit.stock_item_id == stock.id)).first()
...
def test_record_inventory_correction(inventory_system: InventorySystem, session: Session):
    ...
    delta = inventory_system.record_inventory_correction(stock.id, actual_qty=8, reason="lost", author="admin")
    assert delta == -2
    assert stock.quantity == 8
    
    audit = session.exec(select(MaterialAudit).where(MaterialAudit.operation == "correction")).first()
...
def test_request_reorder_triggers_chat(inventory_system: InventorySystem, session: Session):
    ...
    inventory_system.request_material_reorder(mt.id, quantity=50, note="Need more!", author="foreman")
    
    # Check ChatMessage
    msg = session.exec(select(ChatMessage).where(ChatMessage.message_type == ChatMessageType.ORDER)).first()
