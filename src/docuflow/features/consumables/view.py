...
from docuflow.domain.entities.production import Consumable, ConsumableLog
...
class ConsumableView:
    ...
    def __init__(self, system: ConsumableSystem):
        ...
        self.active_consumable: Optional[Consumable] = None
        ...
        with ui.column().classes('w-full gap-6 p-4'):
            ...
            with ui.row().classes('w-full justify-between items-end border-b border-zinc-800 pb-4'):
                with ui.column().classes('gap-1'):
                    ui.label('Учёт расходных материалов').classes('text-2xl font-bold text-zinc-100')
                    ui.label('Мониторинг сопел, линз и вспомогательных материалов').classes('text-sm text-zinc-400')
                
                with ui.row().classes('gap-4'):
                    ui.button('Добавить SKU', icon='add', on_click=self.create_dialog) \
            ...
            with ui.row().classes('w-full gap-6 items-start'):
                ...
                with ui.column().classes('w-80 gap-4'):
                    ui.label('История движений').classes('text-xs font-bold uppercase text-zinc-500 tracking-widest')
                    self.log_container = ui.column().classes('w-full gap-2 border-l border-zinc-900 pl-4')
    ...
    def _render_status_table(self):
        ...
        columns = [
            ...
            {'name': 'name', 'label': 'Наименование', 'field': 'name', 'required': True, 'align': 'left'},
            {'name': 'category', 'label': 'Категория', 'field': 'category', 'align': 'left'},
            {'name': 'quantity', 'label': 'Остаток', 'field': 'quantity', 'align': 'center'},
            {'name': 'min_quantity', 'label': 'Мин. запас', 'field': 'min_quantity', 'align': 'center'},
            {'name': 'actions', 'label': 'Действия', 'field': 'actions', 'align': 'right'},
        ]
        
        self.table = ui.table(columns=columns, rows=[], row_key='id').classes('w-full bg-zinc-900 border-zinc-800 text-zinc-200 shadow-xl')
    ...
    async def refresh_logs(self, consumable_id: Optional[int] = None):
        ...
        if not consumable_id:
            ui.label('Выберите материал для просмотра истории').classes('text-xs text-zinc-600 italic mt-8')
            return
        ...
        for log in logs:
            color = 'emerald-500' if log.operation == 'restock' else 'red-500' if log.operation in ['use', 'write_off'] else 'blue-500'
            with ui.card().classes(f'w-full bg-zinc-900/50 border-l-2 border-{color} p-2 gap-0 shadow-none'):
                ...
                ui.label(f"{'+' if log.qty_delta > 0 else ''}{log.qty_delta:.1f}").classes('text-lg font-mono text-zinc-200 mt-1')
                if log.note:
    ...
    def open_op_dialog(self, row: dict, op_type: str):
        ...
        with ui.dialog() as d, ui.card().classes('bg-zinc-900 border border-zinc-800 w-80'):
            ...
            qty = ui.number(label='Количество', placeholder=f"ед. ({row['unit']})", format='%.2f').classes('w-full')
            note = ui.input(label='Примечание (Задание / Номер заказа)').classes('w-full')
            
            async def submit():
                if qty.value is None or qty.value <= 0: return
                if op_type == 'use':
    ...
    def create_dialog(self):
        ...
        with ui.dialog() as d, ui.card().classes('bg-zinc-900 border border-zinc-800 w-96'):
            ...
            cat = ui.select(['nozzle', 'lens', 'shield', 'gas', 'tape', 'filter', 'other'], label='Категория', value='nozzle').classes('w-full')
            unit = ui.select(['pcs', 'kg', 'm', 'pack', 'liter'], label='Ед. изм.', value='pcs').classes('w-full')
