...
from docuflow.domain.entities.production import Project, WorkItem, WorkItemStatus
...
class ProjectManagementView:
            ...
            with ui.row().classes("w-full gap-4"):
                ...
                with ui.card().classes("w-1/3 p-4"):
                    ...
                    with ui.row().classes("items-center gap-2"):
                        ...
                        ui.button(icon="add", on_click=self._add_project).props("round color=primary")
                ...
                with ui.card().classes("w-2/3 p-4"):
                    ...
                    ui.markdown("Выберите наряды в левой колонке (из проекта Default) и назначьте их в целевой проект.")
    ...
    def _refresh_projects(self):
        ...
        projects = self.project_system.get_all_projects()
        with self.projects_list:
            for p in projects:
                with ui.row().classes("w-full items-center justify-between p-2 hover:bg-blue-50 cursor-pointer rounded"):
                    ...
                    if self.selected_project_id == p.id:
                        ...
                        ui.button(icon="login", on_click=lambda p=p: self._select_project(p.id)).props("flat round")
    ...
    def _add_project(self):
        ...
        if not name:
            ...
            return
        self.project_system.create_project(name)
    ...
    def _refresh_work_items(self):
        ...
        # Filter: only items in Default Project (ID=1)
        filters = WorkItemFilters(project_id=1, limit=500)
    ...
    def _reassign_item(self, wi_id: int):
        ...
        if self.selected_project_id == 1:
            ui.notify("Наряд уже находится в проекте Default", type="warning")
            return
            
        try:
            self.project_system.reassign_work_item(wi_id, self.selected_project_id)
