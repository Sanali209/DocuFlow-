...
class ChatView:
    ...
    async def render_portal(self):
        ...
        with ui.row().classes('w-full h-full gap-0 bg-[#020617]'):
            ...
            with ui.column().classes(f'flex-grow flex-col h-full {self.UI_STYLING["feed_bg"]} relative'):
    ...
    def _build_navigation_sidebar(self):
        ...
        with ui.column().classes(self.UI_STYLING["sidebar_bg"]).classes('w-64'):
            ...
            ui.button('Report Breakdown', icon='error', on_click=self.open_incident_dialog).classes('w-full mt-2 normal-case text-xs').props('flat color=red')
    ...
    def _render_channel_link(self, label: str, icon: str, channel_key: str):
        ...
        with ui.row().classes(f'w-full p-2 rounded-lg cursor-pointer transition-all items-center gap-3 {bg_style} hover:bg-white/5') \
    ...
    def _build_chat_header(self):
        with ui.row().classes('w-full p-6 border-b border-white/5 items-center justify-between'):
            ui.label(f"Channel: {self.active_channel.title()}").classes('text-lg font-bold text-slate-100 uppercase tracking-tighter')
            ui.button(icon='refresh', on_click=self.refresh_discussion_feed).props('flat round size=sm color=slate-500')

    def _build_footer_input(self):
        with ui.row().classes(self.UI_STYLING["input_bar"]):
            self.user_input_area = ui.textarea(placeholder='Write a message...').classes('flex-grow bg-white/5 text-white').props('outlined autogrow dense dark rounded')
            ui.button(icon='send', on_click=self.handle_message_submission).props('unelevated color=primary rounded')
    ...
    async def refresh_discussion_feed(self):
        ...
        if not self.message_feed_container: return
        ...
        if not relevant_messages:
            with self.message_feed_container:
                ui.label('No transmission in this channel yet.').classes('text-slate-600 text-sm mt-10 w-full text-center italic')
            return
    ...
    def _render_message_bubble(self, msg: ChatMessage):
        ...
        with self.message_feed_container:
            ...
            THEME_MAP = {
                ChatMessageType.ORDER: {"bg": "bg-emerald-950/20", "border": "border-emerald-500/20", "icon": "shopping_cart", "color": "emerald"},
                ChatMessageType.INCIDENT: {"bg": "bg-red-950/20", "border": "border-red-500/20", "icon": "error", "color": "red"},
                ChatMessageType.MESSAGE: {"bg": "bg-slate-900", "border": "border-white/5", "icon": "chat", "color": "primary"}
            }
            theme = THEME_MAP.get(msg.message_type, THEME_MAP[ChatMessageType.MESSAGE])
            
            # 2. Render Bubble
            with ui.row().classes(f"{self.UI_STYLING['message_bubble']} {theme['bg']} {theme['border']}"):
                ...
                with ui.column().classes('flex-grow gap-0'):
                    with ui.row().classes('w-full justify-between items-center'):
                        ...
                        ui.label(msg.created_at.strftime('%H:%M')).classes('text-[10px] text-slate-500 font-mono')
    ...
    def open_incident_dialog(self):
        ...
        with ui.dialog() as dialog, ui.card().classes('bg-slate-900 border border-red-500/20 w-96 p-6'):
            ui.label('REPORT BREAKDOWN').classes('text-sm font-black text-red-400 mb-4 tracking-widest')
            
            task_ref = ui.number(label='Task Reference (ID)', format='%d').classes('w-full')
            description = ui.textarea(label='Failure Description', placeholder='Laser tube power loss...').classes('w-full')
            
            async def submit():
                if not description.value: return
                ...
                incident_sys.report_incident(incident_sys.TYPE_BREAKDOWN, description.value, "operator", task_item_id=int(task_ref.value or 0))
