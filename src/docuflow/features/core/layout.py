...
class MainLayout:
    ...
    def build(self, switch_view_fn: Callable):
        ...
        # Header - Fixed Top
        with ui.header().classes('glass-card items-center justify-between px-8 py-4 fixed top-0 w-full z-50'):
            ...
            with ui.row().classes('items-center gap-6'):
                with ui.row().classes('items-center bg-slate-800/50 rounded-full px-4 py-1 border border-white/5'):
                    ui.icon('sensors', size='16px', color='emerald')
                    ui.label(f"NODE: {self._config.node_id}").classes('text-xs font-mono text-emerald-400 font-bold')
                
                with ui.row().classes('items-center gap-2'):
                    ui.avatar(user['username'][0].upper(), color='indigo').classes('text-white font-bold')
                    ui.label(user['username']).classes('font-semibold text-slate-200')
                
                ui.button(icon='logout', on_click=lambda: (nicegui_app.storage.user.clear(), ui.navigate.to('/login'))).props('flat round text-color=slate-400')

        # Sidebar - Fixed Left (Color Matched to Header)
        with ui.left_drawer(value=True).classes('glass-card border-r border-white/5 p-6 pt-24').style('background-color: transparent !important'):
            with ui.column().classes('w-full gap-4'):
                def nav_item(label, icon, view_name):
                    # In this modular approach, we pass view_name to orchestrate routing
                    return ui.button(label, icon=icon, on_click=lambda: switch_view_fn(view_name)).classes('w-full justify-start normal-case text-slate-300 hover:text-white px-4 py-3 rounded-xl hover:bg-white/5 transition-all').props('flat')
        ...
        # Main Content Area
        self.content = ui.column().classes('w-full min-h-screen pt-28 px-10 pb-12 gap-8 bg-[#020617]')
        return self.content
