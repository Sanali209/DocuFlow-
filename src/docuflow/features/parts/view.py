...
class PartLibraryView:
        ...
        with ui.column().classes('w-full gap-6 p-4'):
            ...
            with ui.row().classes('w-full justify-between items-end border-b border-zinc-800 pb-4'):
                with ui.column().classes('gap-1'):
                    ...
                    ui.label('Реестр уникальных SKU с историей использования').classes('text-sm text-zinc-400')
                
                with ui.row().classes('gap-4 items-center'):
                    ui.input(placeholder='Поиск по SKU...', on_change=self.refresh_grid) \
                        .bind_value(self, 'search_query').classes('w-64 bg-zinc-900 border-zinc-800')
                    
                    ui.button('Гео-поиск', icon='straighten', on_click=self.open_geo_search) \
    ...
    async def refresh_grid(self):
        ...
        parts = self.parts_system.list_parts(
            sku_filter=self.search_query if len(self.search_query) > 2 else None,
            limit=50
        )
        
        if not parts:
            with self.grid:
                ui.label('Детали не найдены').classes('col-span-full text-center text-zinc-600 mt-12 py-8 border-2 border-dashed border-zinc-800 rounded-lg')
            return

        with self.grid:
            for part in parts:
                with ui.card().classes('group relative overflow-hidden bg-zinc-900 border-zinc-800 hover:border-blue-500/50 transition-all cursor-pointer p-3') \
                    ...
                    with ui.column().classes('mt-2 gap-0'):
                        ui.label(part.sku).classes('text-xs font-bold truncate text-zinc-200 group-hover:text-blue-400')
                        ui.label(f"{part.bbox_x:.1f} × {part.bbox_y:.1f} мм").classes('text-[10px] text-zinc-500 uppercase tracking-tighter')
                    
                    # Hover Overlay
                    with ui.row().classes('absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity'):
    ...
    def open_part_details(self, part: PartLibrary):
        ...
        with ui.dialog() as dialog, ui.card().classes('w-[900px] max-w-none bg-zinc-950 p-0 overflow-hidden border border-zinc-800'):
            # Header
            with ui.row().classes('w-full bg-zinc-900 border-b border-zinc-800 p-4 justify-between items-center'):
                with ui.column().classes('gap-0'):
                    ui.label(part.sku).classes('text-xl font-bold text-zinc-100')
                    ui.label(f"Версия {part.version} • Впервые замечена: {part.first_seen_at.strftime('%d.%m.%Y')}").classes('text-xs text-zinc-500')
            ...
            with ui.row().classes('w-full p-6 gap-8'):
                ...
                with ui.column().classes('w-1/3 gap-4'):
                    ...
                    with ui.card().classes('w-full bg-zinc-900/50 border-zinc-800 p-4'):
                        ui.label('Метрики').classes('text-xs font-bold uppercase text-zinc-500 mb-2')
            ...
            for wi in items:
                with ui.item().classes('border-b border-zinc-800 last:border-0'):
                    with ui.item_section():
                        ...
                        ui.label(f"ID: {wi.id} • Sidra: {wi.sidra_number or 'N/A'}").classes('text-[10px] text-zinc-500 uppercase')
            ...
            for unit in pallets:
                with ui.item().classes('border-b border-zinc-800 last:border-0'):
                    with ui.item_section():
                        ...
                        ui.label(f"Кол-во: {unit.qty_produced}").classes('text-[10px] text-zinc-500')
                    with ui.item_section().props('side'):
    ...
    def _render_templates(self, sku: str):
        ...
        with ui.column().classes('w-full gap-4'):
            with ui.row().classes('w-full justify-end'):
                ui.button('Добавить заметку', on_click=lambda: self.add_template_dialog(sku)) \
                    .props('small outline color=primary')
            ...
            for tmpl in templates:
                color = 'red-500' if tmpl.severity == 'critical' else 'amber-500' if tmpl.severity == 'warning' else 'blue-500'
                with ui.card().classes(f'w-full bg-zinc-900 border-l-4 border-{color} p-3'):
                    with ui.row().classes('w-full justify-between items-start'):
                        ...
                        ui.button(icon='delete', on_click=lambda t=tmpl: self.delete_template(t)) \
                            .props('flat small color=red')
                    ui.label(f"От {tmpl.created_by} • {tmpl.created_at.strftime('%d.%m.%Y')}").classes('text-[10px] text-zinc-500 mt-2')
    ...
    def add_template_dialog(self, sku: str):
        ...
        with ui.dialog() as d, ui.card().classes('bg-zinc-900 w-96'):
            ...
            sev = ui.select(['info', 'warning', 'critical'], label='Важность', value='info').classes('w-full')
