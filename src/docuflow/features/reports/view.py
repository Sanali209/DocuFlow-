...
class ReportsView:
    ...
    def _render_header(self):
        ...
        with ui.row().classes("w-full items-center justify-between mb-4"):
            ...
            ui.button(
                "Dashboard Docs",
                icon="help_outline",
                on_click=lambda: ui.notify("Analytics documentation is being compiled."),
    ...
    async def refresh_preview_html(self):
        ...
        if not self.html_preview_slot or not self.template_selector:
            return

        try:
            rendered_html = self.report_system.generate_html_preview(
    ...
    async def download_pdf_document(self):
        ...
        try:
            pdf_bytes = self.report_system.generate_pdf_document(
                self.template_selector.value, self.report_parameters
            )
