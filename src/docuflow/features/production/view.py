...
def register_production_view() -> None:
...
async def production_view_wrapper(user: str, system_scope: Any, layout: Any, **kwargs: Any) -> None:
    """Wrapper to instantiate and render the ProductionView."""
    view: ProductionView = ProductionView(user, system_scope, layout=layout)
    await view.render()
...
class ProductionView(BaseDocuWidget):
    ...
    def __init__(self, user: str, system_scope: Any, layout: Any = None) -> None:
        ...
        with ui.column().classes("w-full h-full p-4 gap-4"):
            ...
            with ui.dialog().classes("glass-card p-6") as split_dlg:
                with ui.column().classes("w-[400px] gap-4"):
                    ...
                    src_pallet_id: Any = ui.number().classes("hidden")
                    src_label: Any = ui.label().classes("text-gray-300 font-mono")
                    max_qty: Any = ui.number().classes("hidden")
                    split_qty: Any = (
                    ...
                    async def execute_split() -> None:
                        ...
                        async with self.scope() as req:
                            system: ProductionSystem = await req.get(ProductionSystem)
            ...
            with ui.row().classes("w-full justify-between items-center mt-4"):
                ...
                async def execute_merge() -> None:
                    selected: list[dict[str, Any]] = self.grid.selected
                    if len(selected) < 2:
                    ...
                    target_id: Any = selected[0]["id"]
                    source_ids: list[Any] = [s["id"] for s in selected[1:]]
                    async with self.scope() as req:
                        system: ProductionSystem = await req.get(ProductionSystem)
    ...
    async def handle_shipment(self, pallet_row: dict[str, Any]) -> None:
    ...
    async def refresh_grid(self) -> None:
        ...
        async with self.scope() as req:
            ...
            if self.search_term.value and len(self.search_term.value) >= 2:
                ...
                units: Any = system.db_session.exec(stmt).all()
            else:
                units: Any = system.get_recent_production_units()
