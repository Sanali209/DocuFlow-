...
async def production_view(system: ProductionSystem, current_user: str = "operator") -> None:
    ...
    with ui.column().classes("w-full h-full p-4 gap-4"):
        ...
        grid = ui.table(columns=grid_cols, rows=[], row_key="id", selection="multiple").classes(
        ...
        def refresh_grid():
            if len(search_term.value) >= 2:
                stmt = (
                    ...
                    .where(ProductionUnit.label_id.contains(search_term.value))
            ...
            with ui.column().classes("w-[400px] gap-4"):
                ...
                async def execute_split():
                    if not split_qty.value or split_qty.value >= max_qty.value:
                        ui.notify(
                            "Некорректное количество (должно быть меньше остатка)", type="negative"
                        )
                        return
                    system.split_production_unit(
                        src_pallet_id.value, int(split_qty.value), current_user
                    )
                    split_dlg.close()
                    refresh_grid()
        ...
        with ui.row().classes("w-full justify-between items-center mt-4"):
            ...
            ui.button(
                "СЛИТЬ В ПЕРВУЮ ВЫБРАННУЮ", icon="call_merge", on_click=execute_merge
