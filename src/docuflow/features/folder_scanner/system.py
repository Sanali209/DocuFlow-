...
from docuflow.domain.entities.production import (
    WorkItem, TaskItem, TaskPart, PartLibrary, MaterialType, 
    WorkLog, WorkLogType, WorkItemType, WorkItemStatus
)
from docuflow.features.folder_scanner.parsers.folder import FolderNameParser, FolderMeta
...
class FolderScannerSystem(BaseSystem):
    ...
    def __init__(self, config: Config, sdk: Any, engine: Engine):
        ...
        self._last_successful_scan: Optional[datetime.datetime] = None
    ...
    async def run_full_scan_cycle(self, settings: FolderScannerSettings) -> None:
        ...
        for directory_string, item_type in discovery_targets:
            if not directory_string: continue
    ...
    async def scan_directory_path(self, root_path: Path, work_type: WorkItemType, settings: FolderScannerSettings) -> None:
        ...
        self._log_ingestion_event(f"Discovery complete: found {folders_processed} groups in {root_path.name}")

    async def synchronize_work_item(self, folder: Path, metadata: FolderMeta, work_type: WorkItemType, root_path: Path) -> WorkItem:
        ...
        async with self.sdk.request_scope():
            ...
            with Session(self.db_engine) as db_session:
                existing = db_session.exec(select(WorkItem).where(WorkItem.folder_name == folder.name)).first()
                if existing:
    ...
    async def _process_folder_contents(self, folder: Path, work_item: WorkItem, root_path: Path):
        ...
        gnc_files = [f for f in folder.iterdir() if f.suffix.upper() == ".GNC" and not self.task_filename_parser.is_variant(f)]
    ...
    async def process_task_file(self, gnc_path: Path, work_item: WorkItem, root_path: Path) -> None:
        ...
        with Session(self.db_engine) as db_session:
            task = db_session.exec(select(TaskItem).where(TaskItem.file_path == relative_path)).first()
    ...
    async def resolve_material_mapping(self, parsed_sheet) -> Optional[int]:
        """Maps GNC material codes to the inventory catalog."""
        if not parsed_sheet.mat_code: return None
        async with self.sdk.request_scope():
            inventory_sys = await self.sdk.resolve_system_by_type(InventorySystem)
            mat = inventory_sys.create_material_definition(
                code=parsed_sheet.mat_code,
                thickness=parsed_sheet.thickness
            )
            return mat.id
    ...
    async def resolve_part_mapping(self, part_data, task_id: int, mat_id: int):
        ...
        async with self.sdk.request_scope():
            ...
            with Session(self.db_engine) as db_session:
                part_entry = db_session.exec(select(PartLibrary).where(
                    PartLibrary.sku == part_data.sku, 
                    PartLibrary.version == part_data.version
                )).first()
        ...
        # Task Link
        with Session(self.db_engine) as db_session:
            mapping = db_session.exec(select(TaskPart).where(
