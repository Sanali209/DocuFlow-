...
class FolderScannerSettings(BaseModuleSettings):
    ...
    sidra_scan_path: str = Field(
        default="", description="Network path to SIDRA folder", json_schema_extra={"scope": "local"}
    )
    ...
    poll_interval_seconds: int = Field(
        default=60, description="Scan interval in seconds", json_schema_extra={"scope": "local"}
    )
    ...
    ns_mirror_interval_seconds: int = Field(default=300, json_schema_extra={"scope": "local"})
    ns_mirror_copy_timeout_s: int = Field(default=30, json_schema_extra={"scope": "local"})
