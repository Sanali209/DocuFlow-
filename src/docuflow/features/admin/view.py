            ...
            for u in users:
                ...
                with ui.row().classes(
                    ...
                    with ui.row().classes('items-center gap-4'):
                        ui.avatar(u.username[0].upper(), color='indigo').classes('text-xs font-bold')
...
def render_role_matrix(admin_system: AdminSystem) -> None:
    ...
    MODULES = [
        "bucket", "board", "chat", "workitems", "batching",
        "mat_stock", "consumables", "part_stock", "part_library",
        "scanner", "settings", "reports", "admin"
    ]
    try:
                ...
                with ui.column().classes(f'w-full p-6 bg-white/5 rounded-2xl border {border}'):
                    with ui.row().classes('w-full justify-between items-center mb-4'):
                        with ui.column().classes('gap-0'):
                            ...
                            ui.label(f"Role ID: {r.id}").classes('text-[10px] text-slate-500 font-mono')
                        ...
                        if not is_admin_role:
                            ...
                            ui.button(icon='delete', color='red', on_click=_del_role).props('flat dense')
                        ...
                        for mod in MODULES:
                            ...
                            async def _toggle(m=mod, r_obj=r, active=is_on, mp=module_perm):
                                if r_obj.name.strip().lower() == "admin":
                                    return
                                remaining = [p for p in r_obj.permissions_list if not p.startswith(f"{m}:")]
                                if not active:
                            ...
                            color = (
                                'emerald' if perm_type == 'full' else
                                'indigo'  if perm_type == 'read' else
                                'slate-800'
                            )
                            ui.button(
                                f"{mod}:{perm_type}", on_click=_toggle
                            ).classes(
...
def render_settings_form(
    ...
    try:
        ...
        logger.debug(f"render_settings_form: scope={scope_type}, fields_count={len(fields) if fields else 0}, current_keys={list(current.keys()) if current else []}")
        ...
        with ui.column().classes('w-full mt-4 p-6 bg-white/5 rounded-2xl border border-white/10'):
            ...
            for field_name in fields:
                ...
                with ui.row().classes('w-full items-center justify-between mb-4'):
                    with ui.column():
                        ui.label(field_name.replace('_', ' ')).classes('text-slate-200 font-bold text-sm')
                        if fi.description:
                            ui.label(fi.description).classes('text-[10px] text-slate-500')

                    async def _push(e, f=field_name):
                        admin_system.update_node_setting(node_id or "global", module, f, str(e.value))

                    if fi.annotation is bool:
                        ui.switch(value=str(display_val).lower() == 'true', on_change=_push)
                    elif fi.annotation is int:
                        ui.number(value=int(display_val), on_change=_push).props('dark dense standout')
                    else:
                        ui.input(value=str(display_val), on_change=_push).props('dark dense standout')
...
def render_notifications_form(admin_system: AdminSystem) -> None:
    ...
    try:
        ...
        with ui.column().classes('w-full mt-4 gap-4'):
            for tmpl in tmpls:
                with ui.row().classes('w-full items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10'):
                    with ui.column().classes('gap-1 flex-1'):
                        ...
                        txt_input = ui.input(value=tmpl.text).props('dark standout rounded').classes('w-full text-slate-300')
...
def render_presets_form(admin_system: AdminSystem) -> None:
    ...
    try:
        # Only global presets
        presets = admin_system.session.exec(select(ViewPreset).where(ViewPreset.owner == 'global')).all()
        ...
        with ui.row().classes('w-full mt-4 gap-4 justify-between items-center'):
            ...
            with ui.dialog().classes('glass-card p-4 rounded-3xl') as dialog:
                with ui.column().classes('gap-4 w-[350px] p-6'):
                    ...
                    p_mod = ui.input('Модуль (напр. work_items)').props('dark rounded standout').classes('w-full')
                    p_name = ui.input('Название').props('dark rounded standout').classes('w-full')
                    p_json = ui.input('Пресет (JSON)', value='{}').props('dark rounded standout').classes('w-full')
                    
                    def _create(dlg=dialog):
                        try:
                            preset = ViewPreset(module=p_mod.value, owner='global', name=p_name.value, preset_json=p_json.value)
                    ...
                    ui.button('СОЗДАТЬ', on_click=_create).classes('w-full vibrant-btn rounded-xl h-12')
                    
            ui.button('Добавить', icon='add', on_click=dialog.open).classes('rounded-xl px-6 py-2 vibrant-btn')
            ...
            for preset in presets:
                with ui.row().classes('w-full items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10'):
                    with ui.column().classes('gap-1'):
                        ui.label(f"{preset.name} ({preset.module})").classes('text-lg font-bold text-indigo-400')
    ...
    with ui.tab_panels(tabs, value=t_users).classes('w-full glass-card rounded-b-2xl p-8'):
        ...
        with ui.tab_panel(t_health):
            ...
            cols = [
                ...
                {'name': 'last_active','label': 'Last Activity',  'field': 'last_active','align': 'right'},
            ]
            ...
            ui.button(
                'EMERGENCY STEP DOWN', icon='warning', color='red',
            ...
            with ui.row().classes('w-full justify-between items-center mb-6'):
                ...
                with ui.dialog().classes('glass-card p-4 rounded-3xl') as user_dialog:
                    with ui.column().classes('gap-4 w-[350px] p-6'):
                        ...
                        u_name = ui.input('Username').props('dark rounded standout').classes('w-full')
                        u_pass = ui.input('Password', password=True).props('dark rounded standout').classes('w-full')
                        u_role_sel = ui.select({}, label='Assign Role').props('dark rounded standout').classes('w-full')
            ...
            with ui.row().classes('w-full justify-between items-center mb-6'):
                ...
                with ui.dialog().classes('glass-card p-4 rounded-3xl') as role_dialog:
                    with ui.column().classes('gap-4 w-[350px] p-6'):
                        ...
                        r_name = ui.input('Role Name').props('dark rounded standout').classes('w-full')
                    ...
                    with ui.row().classes(
                        ...
                        with ui.dialog().classes('glass-card p-8 rounded-3xl') as edit_wp:
                            with ui.column().classes('gap-4 w-[400px]'):
                                ...
                                new_nid  = ui.input('Hardware Node ID',               value=w.node_id).props('dark rounded standout')
                                allowed  = ui.input('Allowed Modules (comma-sep)',    value=w.allowed_modules).props('dark rounded standout')
        ...
        with ui.tab_panel(t_conf):
            ...
            if not modules:
                ...
                with ui.row().classes('w-full gap-4'):
                    mod_select = ui.select(modules, value=modules[0], label='Select Module').classes('w-64').props('dark rounded standout')
                    target_node = ui.select(
                        {None: 'Global'}, 
                        value=None, 
                        label='Scope (Node or Global)'
                    ).classes('w-64').props('dark rounded standout')
                ...
                async def _load_conf_nodes():
                    ...
                    try:
                        ...
                        for n in nodes:
                            node_options[n['node_id']] = f"{n['node_id']} ({n.get('status', 'unknown')})"
                ...
                def _refresh_settings():
                    ...
                    logger.debug(f"Config refresh: module={mod_select.value}, scope={scope}, node={node_id}")
