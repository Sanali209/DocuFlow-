...
class NSMirrorStatus(BaseDocuWidget):
    """
    Compact status widget showing local NC synchronization health.
    """

    def __init__(self, engine: Engine, node_id: str, system_scope: Any = None) -> None:
        ...
        async def do_refresh() -> None:
            async with self.scope() as req:
                ...
                if bucket_count == 0:
                    ...
                    self.main_status.classes("text-red-400", remove="text-white")
                    self.sub_status.text = f"{bucket_count} files in total bucket"
                else:
                    self.main_status.text = "Synchronized"
                    self.main_status.classes("text-white", remove="text-red-400")
