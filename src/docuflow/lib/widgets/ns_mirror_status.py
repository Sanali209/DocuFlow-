...
from docuflow.domain.entities.production import WorkerBucketEntry, TaskItem, WorkLog, WorkLogType
    ...
    def build(self):
        with ui.card().classes('w-full bg-slate-900/60 rounded-3xl border border-white/10 p-6'):
            with ui.row().classes('w-full items-center justify-between mb-2'):
                ui.label('LOCAL NS SYNC').classes('text-xs font-bold text-indigo-300 opacity-60 tracking-widest uppercase')
        ...
        try:
            with Session(self.engine) as session:
                ...
                bucket_count = session.exec(
                    select(func.count(WorkerBucketEntry.id))
                    .where(WorkerBucketEntry.node_id == self.node_id)
                ).one()
