...
class WorkItemCard:
        ...
        with ui.dialog() as dialog, ui.card().classes("w-[600px] max-w-none"):
            ...
            with ui.scroll_area().classes("h-[70vh] w-full pr-4"):
                ...
                with ui.grid(columns=2).classes("w-full gap-4 mb-6 p-4 bg-gray-50 rounded"):
                    ...
                    self._info_item(
                        "Путь",
                        self.work_item.folder_path,
                        classes="col-span-2 text-xs text-gray-500 italic",
                    )
    ...
    def _render_tasks_table(self) -> None:
        ...
        with (
            ...
            .classes("w-full shadow-none border")
    ...
    def _render_work_log(self) -> None:
        ...
        logs = self.system.db_session.exec(
            select(WorkLog).where(WorkLog.work_item_id == self.work_item.id)
        ).all()
        if not logs:
        ...
        with ui.column().classes("w-full gap-2"):
            for log in sorted(logs, key=lambda l: l.created_at, reverse=True):
                with ui.row().classes(
                    "w-full items-start gap-4 p-2 border-b border-gray-100 last:border-0"
                ):
                    ui.label(log.created_at.strftime("%H:%M")).classes(
                        "text-caption text-gray-500 w-12"
                    )
    ...
    def _register_document(self, dialog: ui.dialog) -> None:
        ...
        try:
            ...
            ui.notify("Документ зарегистрирован", type="positive")
            dialog.close()
    ...
    def _block_work_item(self, dialog: ui.dialog) -> None:
        ...
        try:
            ...
            ui.notify("Наряд заблокирован", type="warning")
            dialog.close()
        except Exception as e:
            ui.notify(f"Ошибка: {e}", type="negative")
