...
from docuflow.features.core.views import ViewInfo, ViewRegistry  # noqa: E402
...
@asynccontextmanager
async def system_scope() -> AsyncIterator[Any]:
...
async def lifespan(fastapi_app: FastAPI) -> AsyncIterator[None]:
    ...
    try:
        ...
        async with _container() as request_container:
            engine: Engine = await request_container.get(Engine)
            SQLModel.metadata.create_all(engine)

            # 2. Resolve SDK
            sdk_instance: SDK = await request_container.get(SDK)
        ...
        async with _container() as request_container:
            ...
            admin_system: AdminSystem = await request_container.get(AdminSystem)
            registry.init(admin_system)
            logger.info("SettingsRegistry initialized.")

        # 4. Bootstrap P2P
        sdk_instance: SDK = await _container.get(SDK)
        await sdk_instance.on_startup()

        async with _container() as request_container:
            auth_system: AuthSystem = await request_container.get(AuthSystem)
...
async def index_page() -> None:
    user: dict[str, Any] | None = get_current_user()
    if not user:
    ...
    async with _container() as request_container:
        search_system: SearchSystem = await request_container.get(SearchSystem)
    ...
    async def switch_view(view_name: str, **kwargs: Any) -> None:
        ...
        with content_area:
            view_info: ViewInfo | None = ViewRegistry.get_view(view_name)
            if not view_info:
            ...
            async with system_scope() as request_container:
                ...
                if view_info.is_async:
                    res: Any = await view_info.render_fn(*resolved_deps, **kwargs)
                    if hasattr(res, "render"):
                        await res.render()
                else:
                    res: Any = view_info.render_fn(*resolved_deps, **kwargs)
                    if hasattr(res, "render"):
                        res.render()

    # Shared Layout Build
    content_area: Any = layout.build(switch_view)

    # Initial View (Restore from SessionContext if available)
    saved_view: str = SessionContext.get("active_view", "dashboard")
    saved_filter: Any | None = SessionContext.get("active_filter_work_item")
