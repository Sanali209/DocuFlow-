...
from ..parsers.gnc_parser import GNCSheet
...
class GNCGenerator:
    ...
    def _apply_offsets(self, line_text: str, offset_x: float, offset_y: float) -> str:
        ...
        if offset_x != 0:
            line_text = re.sub(
                r"([X])([+-]?\d*\.?\d+)", replace_coord, line_text, flags=re.IGNORECASE
            )

        # Replace Y
        if offset_y != 0:
            line_text = re.sub(
                r"([Y])([+-]?\d*\.?\d+)", replace_coord, line_text, flags=re.IGNORECASE
            )
    ...
    def generate(self, sheet: GNCSheet) -> str:
        ...
        if sheet.header_commands:
            ...
            # Fallback Template
            lines.append("(Generated by DocuFlow GNC Editor)")
            if sheet.metadata.get("model"):
                lines.append(f"(*MODEL {sheet.metadata['model']})")
            if sheet.program_width and sheet.program_height:
                lines.append(
                    f"(*SHEET {sheet.program_width} {sheet.program_height} {sheet.thickness or 0} {sheet.cut_count or 1} 1 0.0 0.0 )"
                )
