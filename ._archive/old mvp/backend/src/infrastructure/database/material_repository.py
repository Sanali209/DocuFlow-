...
from .models import ConsumptionDB, MaterialDB, PartDB, ReservationDB, StockItemDB
    ...
    def list(self, skip: int = 0, limit: int = 100, filters: dict = None) -> list[Part]:
        ...
        if filters:
            if filters.get("search"):
                ...
                query = query.filter(
                    PartDB.name.ilike(search) | PartDB.registration_number.ilike(search)
                )
...
class SQLStockRepository(IStockRepository):
    ...
    def add_reservation(self, reservation: Reservation) -> Reservation:
        ...
        stock = (
            self.db.query(StockItemDB).filter(StockItemDB.id == reservation.stock_item_id).first()
        )
        if stock:
    ...
    def add_consumption(self, consumption: Consumption) -> Consumption:
        ...
        stock = (
            self.db.query(StockItemDB).filter(StockItemDB.id == consumption.stock_item_id).first()
        )
        if stock:
