...
def config_check(service: SettingsService = Depends(get_settings_service)):
    try:
        ...
        from src.infrastructure.config import get_db_path, load_config
        ...
        return {
            "status": "configured"
            if db_ok and not needs_sync_config
            else "needs_config"
            if db_ok
            else "db_error",
...
def test_path(path: str = Body(..., embed=True)):
    print(f"DEBUG: Testing path: '{path}'")
...
def create_assignee(
    name: str = Body(..., embed=True), service: SettingsService = Depends(get_settings_service)
):
...
def create_preset(
    preset: FilterPresetCreate, service: SettingsService = Depends(get_settings_service)
):
