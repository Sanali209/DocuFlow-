...
def update_part(
    part_id: int, data: dict, service: InventoryService = Depends(get_inventory_service)
):
...
def get_part_gnc(
    ...
    try:
        with open(part.gnc_file_path, encoding="utf-8", errors="ignore") as f:
            content = f.read()
        return gnc_service.parse_gnc(content, os.path.basename(part.gnc_file_path))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error parsing GNC file: {e!s}")
...
def read_reservations(
    task_id: int | None = Query(None), service: InventoryService = Depends(get_inventory_service)
):
...
def create_reservation(
    reservation: Reservation, service: InventoryService = Depends(get_inventory_service)
):
...
def delete_reservation(
    reservation_id: int, service: InventoryService = Depends(get_inventory_service)
):
...
def create_consumption(
    consumption: Consumption, service: InventoryService = Depends(get_inventory_service)
):
...
def delete_consumption(
    consumption_id: int, service: InventoryService = Depends(get_inventory_service)
):
