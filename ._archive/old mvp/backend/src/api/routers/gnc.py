...
@router.post("/parse")
async def parse_gnc_file(
    file: UploadFile = File(...), service: GncService = Depends(get_gnc_service)
):
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Failed to parse GNC file: {e!s}")
...
async def save_gnc_file(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to save GNC file: {e!s}")
...
async def get_order_tasks(
    order_id: int, service: ProductionService = Depends(get_production_service)
):
    return service.get_order_tasks(order_id)


@router.post("/save-order-nesting")
async def save_order_nesting(
    data: dict = Body(...), service: ProductionService = Depends(get_production_service)
):
...
async def get_order_nesting_project(
    order_id: int, service: ProductionService = Depends(get_production_service)
):
