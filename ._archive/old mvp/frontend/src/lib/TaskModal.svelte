<script>
    ...
    let name = $state("");
    // let assignee = $state(""); // REMOVED
    let materialId = $state(null);
    let gncFilePath = $state("");
    let loading = $state(false);
    let materials = $state([]);
    let allAssignees = $state([]); // Renamed to avoid confusion with task.assignees
    let selectedAssignees = $state([]); // List of selected IDs
    ...
    async function loadAssignees() {
        allAssignees = await settingService.fetchAssignees();
    }
    ...
    $effect(() => {
        if (isOpen) {
            ...
            if (task) {
                ...
                // Map task.assignees to selected IDs
                if (task.assignees) {
                    selectedAssignees = task.assignees.map((a) => a.id);
                } else {
                    selectedAssignees = [];
                }
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            */
            // Map selected IDs back to objects for the API?
            // Usually API expects IDs for updates, or maybe the list of objects.
            // ProductionService.createJob expects Task Pydantic model which has assignees: List[Assignee]
            // We need to pass the full object or just ID?
            // If the backend expects List[Assignee], we need to find the objects.

            const assigneeObjects = selectedAssignees.map((id) => {
                const found = allAssignees.find((a) => a.id === id);
                return found || { id: id, name: "Unknown" };
            });
                ...
                <div class="form-group">
                    ...
                    <div class="material-row">
                        <select
                            ...
                            style="height: 100px;"
                        >
                            {#each allAssignees as asg}
                                <option value={asg.id}>{asg.name}</option>
                            {/each}
                        </select>
                        <button
                            type="button"
